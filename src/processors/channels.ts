...
import type { IEventProcessorResult } from '../../types';
...
export interface ChannelBanEvent extends BaseChannelEvent, IEventProcessorResult {
    profile: string;
    executor: string;
    isBanned: boolean;
}

export function processChannelCreatedEvent(event: starknet.IEvent): ChannelCreatedEvent {
...
export function processChannelMemberEvent(event: starknet.IEvent): ChannelMemberEvent {
    const data = event.data;
    if (!data) throw new Error('Processor: Expected event with data');
    return {
        channelId: processUint256(data, 0, 1),
...
export function processChannelModEvent(event: starknet.IEvent): ChannelModEvent {
    const data = event.data;
    if (!data) throw new Error('Processor: Expected event with data');
    return {
        channelId: processUint256(data, 0, 1),
...
export function processChannelBanEvent(event: starknet.IEvent): ChannelBanEvent {
    const data = event.data;
    if (!data) throw new Error('Processor: Expected event with data');
    return {
        channelId: processUint256(data, 0, 1),
