...
export function processHandleMintedEvent(event: starknet.IEvent): HandleMintedEvent {
    ...
    return {
        ...
        timestamp: toNumber(Long.fromBigInt(FieldElement.toBigInt(data[data.length - 1]))),
    };
}

export function processHandleBurnedEvent(event: starknet.IEvent): HandleBurnedEvent {
    const data = event.data;
    if (!data) throw new Error('Processor: Expected event with data');
    return {
        localName: FieldElement.toHex(data[0]),
...
export function processHandleLinkedEvent(event: starknet.IEvent): HandleLinkedEvent {
    const data = event.data;
    if (!data) throw new Error('Processor: Expected event with data');
    return {
...
export function processHandleUnlinkedEvent(event: starknet.IEvent): HandleUnlinkedEvent {
    const data = event.data;
    if (!data) throw new Error('Processor: Expected event with data');
    return {
        tokenId: processUint256(data, 0, 1).toString(),
