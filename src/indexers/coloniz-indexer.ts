...
import { ApibaraRuntimeConfig } from "npm:apibara/types";
import { db } from "../../new/db/index.ts";
import * as schema from "../../new/db/schema.ts";

const DAO_AA_CREATED = hash.getSelectorFromName(
  "DaoAACreated",
) as `0x${string}`;
const PROPOSAL_CREATED = hash.getSelectorFromName(
  "ProposalCreated",
) as `0x${string}`;
const PROPOSAL_VOTED = hash.getSelectorFromName(
  "ProposalVoted",
) as `0x${string}`;
const PROPOSAL_CANCELED = hash.getSelectorFromName(
  "ProposalCanceled",
) as `0x${string}`;
const PROPOSAL_RESOLVED = hash.getSelectorFromName(
  "ProposalResolved",
) as `0x${string}`;
  ...
  return defineIndexer(StarknetStream)({
    ...
    async factory({ block: { events }, context }) {
      ...
      const daoCreationEvents = events.map((event) => {
        ...
        logger.log(
          "Factory: new DAO Address    : ",
          `\x1b[35m${daoAddress}\x1b[0m`,
        );
        return {
          address: daoAddress,
          keys: [
      ...
      await db.insert(daoCreation).values(daoCreationData).onConflictDoNothing()
        .execute();
